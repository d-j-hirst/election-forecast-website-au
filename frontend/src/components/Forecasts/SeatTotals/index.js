...
const SeatsRow = props => {
  ...
  if (!props.showCoalition && canShowCoalition && partyAbbr === 'LNP') {
    partyAbbr = 'LIB';
  }
  ...
  return (
    <ListGroup.Item className={styles.seatTotalsItem}>
      ...
      <ProbBarDist
        ...
        valType="integer"
        adjust={true}
        width={Math.min(props.windowWidth - 100, 350)}
      />
    </ListGroup.Item>
  );
};
...
const SeatsRowSet = props => {
  ...
  if (
    ...
    props.results !== null &&
    props.results.overall.seats['LNP'] !== undefined &&
    props.results.overall.seats['NAT'] !== undefined
  ) {
    combinedCoalition =
      props.results.overall.seats['LNP'] + props.results.overall.seats['NAT'];
  }
  return (
    <>
