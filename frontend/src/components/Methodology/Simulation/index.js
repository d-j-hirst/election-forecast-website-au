...
const MethodologyPollTrend = props => {
    return (
        <>
            ...
            <p>
                As of September 2022, for state elections, a further adjustment to the TPP for each seat is made
                to represent the fact that state election swings tend to correlate with federal election swings
                in the same areas. Analysis of the 2022 SA, 2019 NSW and 2018 Victorian elections found a
                statistically significant correlation between the swing in that seat and the
                estimated federal swing within the boundaries of the state seat (after accounting for the candidate
                effects noted in the previous section). The size of this effect fit
                a decaying curve where about 55% of the federal swing would be seen in a state election at the
                same time, down to 33% for a state election 6 months away from the federal election (such as the 
                2022 Victorian election). Given
                the small number of elections used to form this conclusion, and the likelihood of significant
                variability between elections in the size of this effect, in each simulation the actual effect
                size is generated by a gamma distribution calibrated so that the median effect size is as above.
                This allows the actual size in any individual simulation to range from almost zero to actually
                having the state election exaggerate the federal swings.
            </p>
            <p>
            ...
            <p>
                ...
                reliable to simulate. However, because this is a comprehensive model, and because some
                contests may not result in a TPP contest, FP votes are also simulated. This process is quite
            ...
            <p>
                FP votes for non-major parties including independents and a generic "others" category are simulated first.
                The behaviour of FP votes in historical elections is analysed separately for each party category
                (see "Fundamentals" above) and then the results are applied to the vote for those parties in the
                previous election. This includes, where appropriate, changes from previous election
            ...
            <p>
                For independents who are either incumbent, or obtained a substantial vote share in the previous election,
            ...
            <p>
                ...
                Seat betting odds and seat polls are used here to help inform of the likely performance
                of the prominent independent candidates. For polls a considerable margin of error is used due to the
            ...
            <p>
                ...
                historical relationship between Greens FPs, Liberal FPs and the incidence of Green seat wins,
                an estimate for the Green FP is determined from the seat betting data. A distribution based on this
                estimate is mixed equally with the original distribution (without betting odds)
                to create an new estimated distribution for the greens FP vote that includes betting odds data.
            </p>
            ...
            <p>
                ...
                in between for the purposes of determining where they might be most popular.
                (Some might wonder why the lack of a left-wing "populist" party - basically, there has
                never really been one so there's no way to assess where they might be most popular.)
            </p>
            ...
            <p>
                ...
                projections heavily reduced just because the generic Others vote happened to be too high.
            </p>
