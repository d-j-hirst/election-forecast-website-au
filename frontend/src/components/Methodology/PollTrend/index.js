...
const MethodologyPollTrend = props => {
  return (
    <>
      ...
      <p>
        The first step of preparing the forecast is to generate a
        <i>
      ...
      <p>
        <GlossaryLink word="Poll" /> results have been collected from public
      ...
      <ul>
        <li>
          cover the entire region for the election they are being run for (so,
          for example, a poll for a federal election that only covers one state
          is not included)
        </li>
        <li>
        ...
        <li>
          report/imply whether undecided voters are in the sample and if so,
      ...
      <ul>
        ...
        <li>
          ...
          substantially different methodologies under the same brand name are
          classed separately (for example, there are different entries for Roy
          Morgan face-to-face, phone, SMS and multi-mode polls.)
        </li>
        <li>
          ...
          recorded for display purposes but is not used in the forecast
          ...
          &quot;Others&quot; it is assumed to be the number required to make all
          the first preferences add up to 100.
        </li>
      </ul>
      <h5 id="poll-data-used">Generating the poll trend</h5>
      <p>
        From this data the actual poll trend is generated. This is done using{' '}
      ...
      <p>
        ...
        page. (Important note: the poll trend is <i>not</i> actually the same
        data as this published vote trend, it just has a similar form. Further
        adjustments are made as described later.)
      </p>
      <p>The following assumptions are used to formulate the model:</p>
      <ul>
        <li>
          The real voting intention for the whole election is assumed to change
          over time approximately following a{' '}
          <ExtLink href="https://en.wikipedia.org/wiki/Random_walk#Gaussian_random_walk">
            Gaussian random walk
          </ExtLink>
          . Changes in voting intention are assumed to mostly be small most of
        ...
        <li>
          ...
          to actual changes in voting intention. On the other hand systemic bias
          is expected as a result of the inability of methods to perfectly
      ...
      <p>
        ...
        &quot;others&quot; for all minor parties except the Greens. A trend is
        generated for this entire time period, even for minor parties that
        don&apos;t have any polling data for a long part of this period.
      </p>
      <p>
        A polling trend is also calculated for the two-party-preferred (TPP)
        vote. This does <i>not</i> use published TPP values, but instead
        ...
        the date of the poll is used for the TPP calculation, and the
      ...
      <p>
        In order to best make use of polling information, it&apos;s important
        for the model to determine the value of each pollster&quot;s results in
        estimating true voting intention. The most natural way to do this is to
        compare pollsters&quot; results to actual election results.
        Unfortunately, most Australian pollsters have been tested at extremely
        few elections; with such a low sample size, the results of such a
        comparison won&apos;t have much meaning. Therefore assessing pollsters
        needs to be done in a somewhat more indirect manner, by measuring
        pollsters and polls <i>against each other</i>.
      </p>
      <p>
      ...
      <ul>
        ...
        <li>
          Second, use that poll <i>trend</i> to estimate the distribution of
          final results, calibrated by the relationship between past poll trends
          and results.
        </li>
      </ul>
      <p>
      ...
      <ul>
        <li>
          How good the pollster is as an <i>indicator of trend</i>? That is,
          after taking away a constant house effect, does it follow other polls
          closely, or does it have noisy results or trends that other pollsters
          don&apos;t show?
        </li>
        <li>
          What is the{' '}
        ...
        <li>
          ...
          overall bias of the poll trend, it&apos;s much better to use pollsters
          with a consistent bias, even if quite large, rather than those that
          change a lot from one election to the next.
        </li>
      </ul>
      <h6 id="pollster-calibration">Setting up the comparison poll trends</h6>
      <p>
        In order to measure these, a simplified <i>comparison poll trend</i> is
        generated for each pollster in each election. This pollster shall
        henceforth be referred to as the <i>pollster in focus</i> - i.e. the one
        whose behaviour is being measured. This is done in order to compare that
        pollster to the trend from the <i>other</i> pollsters. This follows a
        similar approach as for the final poll trend described above, with the
        following changes:
      </p>
      <ul>
        <li>
      ...
      <p>
        ...
        the polls of the pollster in focus in order to determine how good it is
        as an <i>indicator of trend</i>. Before calculating this, the house
        effect of the pollster in focus is subtracted from each of their polls
        (for this step it is the movement of the polls, not the absolute
        position, that matters).
      </p>
      <p>
        Then, the differences from each of those polls from the poll trend
        (generated from the other polls) are averaged to give a measure of the
        pollster&apos;s indication of trend for this election. In order to
        prevent a pollster from being penalised for polling when no other
        pollsters are polling (and potentially getting a result far off trend
        when the trend would have been much closer had there been other polls),
        this average is weighted by the proximity to other polls. While the
        exact formula won&apos;t be discussed here, to give a couple of
        examples, having two other polls on the same day gives a full weighting,
      ...
      <p>
        ...
        (but not necessarily compared to the &quot;true&quot; voting intention).
        It&apos;s important to be cautious in the interpretation of this
        variable - a low value may be as a result of the pollster herding, or
      ...
      <p>
        ...
        election, they are combined (separately for each pollster) in a weighted
        average. The weightings for each election are determined by both the
        number of polls for the pollster in focus, and also the number of polls
        in total (in both cases, a high number of polls gives a higher
        weighting). Additionally to the actual results, this weighted average
        includes an &quot;initial&quot;election (not a real result) that is
        functionally similar to (but mathematically different to) a Bayesian
        prior - it allows for a default expectation of poll behaviour when there
        is little or no empirical data available. This default election is
        equivalant to the performance of a quite erratic pollster that recorded
        about 7 polls. As the pollster records more actual polls, the influence
        of this default value will become insignificant. But for pollsters with
        little data available to calibrate their polls, it ensures that the
        model is cautious about the influence that their results have.
      </p>
      <h6 id="pollster-calibration">
        Calculating the typical bias and its variability
      </h6>
      <p>
        As alluded to before, the most natural way to measure a pollster&apos;s
        long-term bias is to compare its final results to the actual election
        results. The problem with this in Australia is that most pollsters
        currently have a very limited track record, often having final
        pre-election polls in only a handful of elections. As a result, the
        random variation inherent to opinion polling is likely to overwhelm any
        signal in those results.
      </p>
      <p>
        As a more robust alternative, the model uses the entire set of polls for
        each election to reduce this randomness. This is taken by taking the
        estimated house effect for that pollster, relative to the trend for
        <i>all</i> pollsters, and adding it to the final median value of that
        poll trend. This creates a <i>final result estimate</i> for that
        pollster with much reduced noise, since for any well-polled election
        both values in this calculation are drawn from a sample of multiple
        results. (In the case of the house effect, all of the pollster&apos;s
        polls in the term prior to the election, and in the case of final poll
        value, all the polls from any pollster close to the election.)
      </p>
      <p>
      ...
      <p>
        ...
        average of these calculated biases for elections for which the pollster
        conducted polls. Once again, this weighting is adjusted according to the
        number of polls that the pollster conducted, as well as the total number
        of polls conducted overall in that election, in order to reflect that
        such estimates are more reliable with a larger sample of data to draw
        from. Finally, the <i>bias variability</i> is measured using the
        standard deviation of the same data with the same weightings.
      </p>
      <p>
